...
class AlbumsFactory(factory.django.DjangoModelFactory):
    ...
    class Meta:
        model = Albums
        django_get_or_create = ('profile', 'published', 'cover',)
...
class TestStream(TestCase):
    ...
    def setUp(self):
        ...
        self.sally.save()
        self.b = PhotoFactory(profile=self.sally.profile, published='pub', image='b.jpg')
        self.b.save()

    def test_stream_view(self):
        self.client.login(username='bob', password='test')
        response = self.client.get('/stream/', follow=True)
        im = get_thumbnail('a.jpg', "400x400", crop="center")
    ...
    def test_stream_follow_and_block(self):
        ...
        self.client.login(username='bob', password='test')
        response = self.client.get('/stream/', follow=True)
        im = get_thumbnail('b.jpg', "400x400", crop="center")
        self.assertIn(im.url, response.content)
        self.bob.profile.block(self.sally.profile)
        response = self.client.get('/stream/', follow=True)
...
class TestLibrary(TestCase): 
    def setUp(self):
        ...
        self.c = AlbumsFactory(profile=self.bob.profile, published='pub', cover=self.a)
        self.c.photos.add(self.a)
        self.c.photos.add(self.b)
        self.a.save()
        self.b.save()
        self.c.save()
    ...
    def test_library(self):
        ...
        im = get_thumbnail('b.jpg', "100x100", crop="center")
        self.assertIn(im.url, response.content)

    def test_l_albums(self):
        self.client.login(username='bob', password='test')
        response = self.client.get('/library/', follow=True)
        im = get_thumbnail('a.jpg', "100x100", crop="center")
        self.assertIn(im.url, response.content)
